...
import { deleteQuestionBankPointsByDocument } from "./qdrant_client.js";
...
async function assertDocumentsReady(documentIds: string[]) {
...
export const generateFromDocumentsHandler = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
    try {
        ...
        await assertDocumentsReady(parsed.data.documentIds);
        const groups = await generateQuestionsFromDocuments(parsed.data);
...
export const searchQuestionBankHandler = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
    try {
        ...
        const results = await searchQuestionBank(parsed.data.query, {
            organisationId: req.user?.organisationId ?? null,
