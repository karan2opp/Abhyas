...
export default function AssignmentDetailPage() {
  ...
  const addTopic = () => {
    ...
    if (!topic) return;
    if (aiTopics.some((t) => t.toLowerCase() === topic.toLowerCase())) {
      setAiTopicInput("");
      return;
    }
    if (aiEditingQuestionId && aiTopics.length >= 1) {
      setAiTopicInput("");
      return;
    }
    setAiTopics((prev) => [...prev, topic]);
    setAiTopicInput("");
  };

  const removeTopic = (index: number) => {
    setAiTopics((prev) => prev.filter((_, i) => i !== index));
  };
  ...
  const handleGenerateWithAi = async () => {
    ...
    try {
      if (aiEditingQuestionId) {
        ...
        const topics = aiTopics.map((t) => t.trim()).filter((t) => t !== "");
        if (topics.length === 0) {
        ...
        const generatedQuestions: any[] = Array.isArray(generated?.blocks)
          ? generated.blocks.flatMap((b: any) => (Array.isArray(b.questions) ? b.questions : []))
          : [];
        if (generatedQuestions.length === 0) {
          throw new Error("No questions were generated by the AI agent.");
        }

        toast.success(`Successfully generated ${generatedQuestions.length} questions. Saving...`);
            ...
            <div className="space-y-4 py-2 max-h-[65vh] overflow-y-auto custom-scrollbar">
              ...
              <div className="space-y-1.5">
                <label className="text-sm font-semibold text-gray-300">
                  {aiEditingQuestionId ? "Topic" : "Topics"}
                </label>
                <div className="flex flex-wrap items-center gap-2 bg-[#14151f] border border-white/15 rounded-xl p-2 focus-within:border-white/30 transition-all">
                  {aiTopics.map((t, i) => (
                    <span
                      key={i}
                      className="inline-flex items-center gap-1.5 px-2.5 py-1 bg-orange-600/20 border border-orange-500/40 rounded-full text-xs font-medium text-orange-200"
                    >
                      {t}
                      <button
                        type="button"
                  ...
                  <input
                    ...
                    onKeyDown={(e) => {
                      if (e.key === "Enter") {
                        e.preventDefault();
                        addTopic();
                      } else if (e.key === "Backspace" && aiTopicInput === "" && aiTopics.length > 0) {
                        removeTopic(aiTopics.length - 1);
                      }
                    }}
                    onBlur={addTopic}
                    placeholder={
                    ...
                    className="flex-1 min-w-[140px] bg-transparent outline-none text-white placeholder:text-zinc-500 text-sm px-1"
                  />
                  <Button
                    type="button"
                    size="sm"
                    ...
                    onClick={addTopic}
                    disabled={!aiTopicInput.trim() || (Boolean(aiEditingQuestionId) && aiTopics.length >= 1)}
                  >
                    <Plus className="h-3.5 w-3.5 mr-1" /> Add Topic
                  </Button>
                </div>
                {!aiEditingQuestionId && (
                  <p className="text-xs text-gray-500">Add each topic as its own chip — no commas needed.</p>
                )}
              </div>
