...
export async function getUsableBook(bookId: string, access: QuestionBankAccess, isSystemAdmin = false) {
    const book = await getBook(bookId);
    if (!book || (!isSystemAdmin && !canAccessDocument(book, access))) throw ApiError.notFound("Book not found");
